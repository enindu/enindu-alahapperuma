...
@section('content')
    <div class="card">
        <div class="card-body">
            <div class="x-content">
                <h1>Playing With DNS Clients</h1>
...
$ dig @8.8.8.8 enindu.com +notcp # Use UDP</code></pre>
                ...
                <p>By now, you're familiar with DNS clients and the ports used by DNS servers. If you've read my previous article, you already understand how a DNS client works under the hood, so there's no need to revisit that. Instead, let's explore different scenarios for building a standalone DNS client from scratch.</p>
                <p>First, I want to mention that I'll be using the Go programming language to demonstrate these scenarios. I'll take a high-level approach by leveraging an external DNS library instead of manually crafting DNS queries. While manually creating DNS queries can be insightful, it adds unnecessary complexity for this demonstration, where the focus is on the functionality of a DNS client. For these examples, I'll be using the <a href="https://github.com/miekg/dns" target="_blank">https://github.com/miekg/dns</a> library.</p>
                <p>As you know, when you visit a website using a domain name, the local DNS resolver first queries a recursive resolver. Now, let's build a simple command-line DNS client that retrieves the authoritative name server for a given domain.</p>
                <pre><code>var (
   domain            string
...
fmt.Printf("%s\n", response.String())</code></pre>
                <p>This code sends a DNS request to the specified recursive resolver to retrieve NS records, which identify the authoritative name servers for the given domain.</p>
                <p>You can ignore "Point 1", as it simply defines command-line flags—a standard part of Go. When running this application, you must specify the -d flag for the domain you want to look up. The -rr flag is optional; by default, the application uses Google's DNS server. If you prefer a different DNS server, you can define it using this flag.</p>
                <p>In "Point 2", we create the DNS client, which will allow us to perform DNS message exchanges in the future. This client handles the communication between our application and the DNS servers.</p>
                <p>In "Point 3", we create the DNS message. I'd like to add a bit more detail here. While I covered the general structure of a DNS request in my previous article, I didn't go into the specifics of each field. I'm not going to cover all the fields in this case, but since we'll be using the question field, it's worth mentioning that it has three subfields.</p>
                <ul>
                ...
                <p>In "Point 4", we exchange the DNS message with the recursive resolver and receive the response. In this example, I print the response in a dig-like format, but in reality, you can process the response however you like. If the response is successful, the answer field of the DNS response typically contains the authoritative name server when querying a recursive server.</p>
                <p>Now we know that sending a DNS request to a recursive resolver is fairly simple and straightforward using a library like miekg/dns. Additionally, in DNS resolution, we can bypass the recursive resolver and send DNS requests directly to the root servers.</p>
...
Address: 2001:503:ba3e::2:30</code></pre>
                <p>As shown in the example above, the IPv4 address for a.root-servers.net is 198.41.0.4.</p>
                <p>Now that we have the IP address for a.root-servers.net, we can use this information to create a DNS client that communicates directly with the root servers. The code structure will follow the same approach as the previous example.</p>
                <pre><code>var domain string
...
for _, rootServer := range rootServers {
   ...
   fmt.Printf("%s\n", response.String())
                ...
                <p>In "Point 1", we added all the IP addresses of the 13 root servers in a slice. While this isn't strictly necessary, sending a query to any single root server will work because they are all interconnected and will provide the relevant answers regardless of which root server we use. I've included all the IP addresses here for your reference.</p>
                <p>In "Point 2", we iterate through the root servers slice and retrieve DNS responses from each root server. We iterate because I included all the root servers in the slice.</p>
                <p>If you run this on your own, you'll notice that the root servers do not respond with an answer field. This is because root servers do not have information about authoritative name servers, but rather they contain information about TLD servers. In general, when you send DNS requests to root or TLD servers, there won't be an answer section in the response. Instead, you'll need to check the authority section. As the next step, we need to send a DNS request to a TLD server to retrieve the authoritative name servers.</p>
                <pre><code>var domain string
...
fmt.Printf("%s\n", tldServerResponse.String())</code></pre>
                <p>Here's what our code looks like. It's fairly long, so let me explain it step by step.</p>
                <p>In "Point 1", since it's unnecessary to iterate over every root server, we send the DNS request directly to a.root-servers.net.</p>
                <p>In "Point 2", we create the hostname of the TLD, specifically the first TLD server that the root server provides. Just like with the root servers, it's unnecessary to iterate over every TLD server. We only need one TLD server in this case, and it will give us the authoritative name server we're looking for.</p>
                <p>For a .com domain, there are 13 TLD servers worldwide, just like the root servers. The number of TLD servers may vary for different TLDs.</p>
                <p>In "Point 3", we retrieve the IP addresses of the TLD server. Typically, there will be two IP addresses: one for IPv4 and one for IPv6. We need to handle this manually in the code because different TLDs have different IP addresses.</p>
                <p>In "Point 4", we perform the DNS exchange and receive the response from the TLD server. As you can see when running the code, the response will not have an answer field, just like in the previous code. Instead, the authority field contains the information about the authoritative server.</p>
                <p>If we already know the authoritative name server, we can send a DNS request directly to it to retrieve the DNS records of a domain name. This will be the final scenario that I'll demonstrate.</p>
                <pre><code>var (
   domain                  string
...
Discovered open port 53/tcp on 49.12.121.55</code></pre>
                <p>So we found 15 open DNS ports. We don't know yet whether these are recursive resolvers, authoritative name servers, or misconfigured DNS servers, also known as open resolvers. Let's find out by sending DNS requests. Let's create a small program using Go.</p>
                <pre><code>servers := []string{
   "49.12.121.225",
   "49.12.121.101",
   "49.12.121.59",
   "49.12.121.38",
