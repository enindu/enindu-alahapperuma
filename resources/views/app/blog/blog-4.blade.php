...
@section('content')
    <div class="card">
        <div class="card-body">
            <div class="x-content">
                <h1>Playing With HTTP Servers</h1>
    ...
    <div class="card">
        <div class="card-body">
            <div class="x-content">
                <p>First, let me explain why I chose the Go language for these implementations. I use Go because I am highly familiar with it, and its ecosystem—including the standard library and external packages—provides excellent support for network-level programming. Additionally, Go's concurrency model makes it highly efficient at handling millions of requests per second with ease. This makes Go particularly well-suited for tasks like server-side development, especially when building HTTP servers.</p>
                <p>Since my focus is on programming rather than the language itself, I won't go into detailed explanations of language-specific features. Additionally, I will approach this topic from the highest level down to the lowest level possible. So, let's begin at the highest level.</p>
                <p>In these examples, I used the standard library to create servers.</p>
                <pre><code>http.HandleFunc("/", func(rw http.ResponseWriter, r *http.Request) {
    // Point 1
})
...
if err != nil {
    panic(err)
                ...
                <p>This is what a basic HTTP server looks like in Go. Pretty simple, right? However, there are many considerations to take into account before deploying this into production, though I don't intend to discuss that at the moment. Let me explain how this works.</p>
                <p>Assuming you're familiar with the OSI model, I'm referring to the application layer. HTTP is an application layer protocol. When someone sends an HTTP request to the "/" path on this server, nothing will happen, obviously, because I haven't added any functionality yet. However, my focus here is on explaining how to implement that functionality.</p>
                ...
                <p>At "Point 1", you can read the data from the request, generate the appropriate response, and send it back to the client. The final code will look something like this.</p>
                <pre><code>http.HandleFunc("/", func(rw http.ResponseWriter, r *http.Request) {
    // Point 1
...
if err != nil {
    panic(err)
                ...
                <p>As you can see, at "Point 1", we only checked if the method is correct, and if it is, we send the response right away at "Point 2". If you run this and visit "127.0.0.1:5000" in your web browser, you'll see the "Hello world!" message. If you open the developer tools and inspect the response, you'll see a properly formatted HTTP message like this.</p>
...
Hello world!</code></pre>
                ...
                <p>Now, let me take it a step lower. To achieve this, we need to create a TCP server.</p>
                <pre><code>listener, err := net.Listen("tcp", "127.0.0.1:5000")
...
for {
    ...
    go func(c net.Conn) {
        // Point 3
                ...
                <p>It's still pretty simple, right? This is not an HTTP server, but a lower-level TCP server. If you run this and navigate to "127.0.0.1:5000" in your web browser, the browser will never stop loading because the browser is an HTTP client and expects to connect with an HTTP server. An HTTP server needs to follow a set of rules, and this application does not adhere to those rules. I'll explain this in more detail below.</p>
                <p>Before explaining this code, I'd like to quickly mention something. Since this is the structure of a TCP server, you can implement any TCP-based protocol, such as SSH, SMTP, FTP, etc., using this same structure.</p>
                ...
                <p>In "Point 2", we handle requests concurrently. There can be several requests—potentially even hundreds—for a single connection. For example, imagine serving a typical web page on this server, which has multiple assets. Each asset would generate a separate request. If we only handled a single request without concurrent threads, the assets of that page would not be served. That's why we open requests in multiple concurrent threads.</p>
                <p>Now comes the interesting part, "Point 3". This is where we need to implement the HTTP rules. For a minimal working example,</p>
                <ul>
                ...
                <p>Please note that this example will not work for HTTP/2 or HTTP/3 versions, which are widely used these days. In fact, even my own website is served using the HTTP/3 version.</p>
                <p>After verifying all the HTTP rules in "Point 3", we need to serve the relevant response back to the client. To do this, we must respect and follow the HTTP protocol thoroughly. Unlike the high-level example, we need to handle this manually. We need to construct a proper HTTP message and send it back to the client. The HTTP message will look something like this:</p>
                <pre><code>HTTP/1.1 200 OK
Content-Length: 12
Content-Type: text/plain; charset=utf-8
...
writer.WriteString(string([]byte{0xF0, 0x9D, 0x92, 0x9C}) + "\n")</code></pre>
                ...
                <p>This article has already become quite long, so I will continue discussing the lowest-level HTTP server in a separate article. You can find the code I used for this demonstration at <a href="https://github.com/enindu/examples-for-blog/tree/master/playing-with-http-servers" target="_blank">https://github.com/enindu/examples-for-blog/tree/master/playing-with-http-servers</a>.</p>
            </div>
        </div>
    </div>
@endsection
